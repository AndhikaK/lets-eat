      ...
      <div className="relative p-4 w-full max-w-2xl h-full md:h-auto">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          <div className="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Liked Recipes</h3>
            <button
              type="button"
          ...
          <div className="p-6 space-y-6">
            <div className="grid lg:grid-cols-2 gap-5">
              {likedContext.likedState.length > 0 ? (
                likedContext.likedState.map((liked) => {
                  return <LikedRecipe key={liked.uri} data={liked} />;
                })
              ) : (
                <div>You have not liked any recipe</div>
              )}
            </div>
          </div>
        </div>
